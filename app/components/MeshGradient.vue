...
// ── Full desktop shader with drift, ripples, specular ──
const FRAG = `
precision highp float;
...
vec2 warpUv(vec2 uv, float time) {
  float slow = time * 0.08;
  vec2 base = uv * 3.0 + u_seed;
  float warpX = fbm(base + vec2(slow * 0.7, slow * 0.3)) - 0.5;
  float warpY = fbm(base + vec2(slow * -0.5, slow * 0.9) + 50.0) - 0.5;
  return uv + vec2(warpX, warpY) * 0.03;
}

vec2 driftPos(vec2 origin, float id, float time) {
  float slow = time * 0.15;
  float dx = fbm(vec2(id * 7.3 + u_seed, slow)) - 0.5;
  float dy = fbm(vec2(id * 13.1 + u_seed, slow + 40.0)) - 0.5;
  return origin + vec2(dx, dy) * 0.4;
}

float driftScale(float id, float time) {
  return 0.75 + 0.5 * fbm(vec2(id * 5.7 + u_seed + 20.0, time * 0.1));
}

float driftStretch(float id, float time) {
  return 0.7 + 0.6 * fbm(vec2(id * 9.2 + u_seed + 60.0, time * 0.12));
}

float driftAngle(float id, float time) {
  return fbm(vec2(id * 3.8 + u_seed + 80.0, time * 0.08)) * 6.28;
}
...
void main() {
  ...
  vec2 warped = mix(uv, warpUv(uv, u_time), u_intensity);
  ...
  float i = u_intensity;
  vec2 p1 = mix(o1, driftPos(o1, 1.0, u_time), i);
  vec2 p2 = mix(o2, driftPos(o2, 2.0, u_time), i);
  vec2 p3 = mix(o3, driftPos(o3, 3.0, u_time), i);
  vec2 p4 = mix(o4, driftPos(o4, 4.0, u_time), i);
  vec2 p5 = mix(o5, driftPos(o5, 5.0, u_time), i);
  vec2 p6 = mix(o6, driftPos(o6, 6.0, u_time), i);
  vec2 p7 = mix(o7, driftPos(o7, 7.0, u_time), i);

  float rScale = mix(0.7, 1.0, smoothstep(0.5, 1.2, u_aspect)) * 0.87;

  float b1 = blob(warped, p1, 1.30 * rScale * mix(1.0, driftScale(1.0, u_time), i), 1.4, mix(1.0, driftStretch(1.0, u_time), i), driftAngle(1.0, u_time) * i);
  float b2 = blob(warped, p2, 0.85 * rScale * mix(1.0, driftScale(2.0, u_time), i), 2.2, mix(1.0, driftStretch(2.0, u_time), i), driftAngle(2.0, u_time) * i);
  float b3 = blob(warped, p3, 1.20 * rScale * mix(1.0, driftScale(3.0, u_time), i), 1.5, mix(1.0, driftStretch(3.0, u_time), i), driftAngle(3.0, u_time) * i);
  float b4 = blob(warped, p4, 1.15 * rScale * mix(1.0, driftScale(4.0, u_time), i), 2.0, mix(1.0, driftStretch(4.0, u_time), i), driftAngle(4.0, u_time) * i);
  float b5 = blob(warped, p5, 1.00 * rScale * mix(1.0, driftScale(5.0, u_time), i), 1.8, mix(1.0, driftStretch(5.0, u_time), i), driftAngle(5.0, u_time) * i);
  float b6 = blob(warped, p6, 1.15 * rScale * mix(1.0, driftScale(6.0, u_time), i), 1.4, mix(1.0, driftStretch(6.0, u_time), i), driftAngle(6.0, u_time) * i);
  float b7 = blob(warped, p7, 1.05 * rScale * mix(1.0, driftScale(7.0, u_time), i), 1.4, mix(1.0, driftStretch(7.0, u_time), i), driftAngle(7.0, u_time) * i);

  float ei = i * i * (3.0 - 2.0 * i);

  vec3 col = base;
  col = mix(col, teal,    b1 * 0.85 * ei);
  col = mix(col, navy,    b2 * 0.5 * ei);
  col = mix(col, indigo,  b3 * 0.55 * ei);
  float b4dist = clamp(length((warped - p4) * vec2(u_aspect, 1.0)) / (0.85 * rScale), 0.0, 1.0);
  vec3 amberGrad = mix(amber, amberEdge, smoothstep(0.12, 0.55, b4dist));
  col = mix(col, amberGrad, b4 * 0.85 * ei);
  col = mix(col, sunset,  b5 * 0.70 * ei);
  col = mix(col, magenta, b6 * 0.40 * ei);
  col = mix(col, pink,    b7 * 0.35 * ei);

  float midWash = blob(warped, vec2(0.45 + 0.1 * hash(vec2(8.0, u_seed)), 0.40 + 0.2 * hash(vec2(18.0, u_seed))), 1.0, 1.8, 1.0, 0.0);
  col = mix(col, purple, midWash * 0.15 * ei);

  vec2 glowCenter = vec2(0.5, -0.15);
  float glowDist = length((uv - glowCenter) * vec2(u_aspect * 0.3, 0.8));
  float glowMask = 1.0 - smoothstep(0.0, 1.2, glowDist);
  vec3 glowColor = vec3(0.10, 0.08, 0.22);
  col = mix(col, col + glowColor, glowMask * (1.0 - ei));

  col = pow(col, vec3(1.7));
  col *= 0.60;
  col *= 1.0 + 0.03 * sin(u_time * 0.08) * ei;

  vec3 vigColor = pow(vec3(0.067, 0.063, 0.137), vec3(2.2));
  float vigDist = length((uv - 0.5) * vec2(u_aspect * 0.55, 1.0));
  float vigMask = smoothstep(0.0, 0.55, vigDist);
  vigMask *= vigMask;
  float vigStrength = mix(0.3, 0.85, ei);
  col = mix(col, vigColor, vigMask * vigStrength);
...
// Single-frame draw — used by mobile and as the core of the desktop loop
function renderOnce() {
...
// Desktop-only continuous animation loop
function renderLoop() {
  if (!gl || !program) return;
  resize();
...
function handleClick(e) {
  ...
  const isDouble = now - lastRippleMs < 500;
  lastRippleMs = now;
  taps.push({ x, y, time: elapsed, force: isDouble ? 2.0 : 1.0 });
